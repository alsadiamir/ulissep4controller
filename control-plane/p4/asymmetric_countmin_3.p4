...
control MyIngress(inout headers hdr,
    ...
    register<bit<48>>(1024) pkt_count_2;
    ...
    apply {
        if (hdr.ipv4.isValid()) {
            ...
            bit<32> flow2;
            bit<32> flow_opp2;
            bit<48> last_pkt_cnt2;
            bit<48> last_pkt_cnt_opp2;
            ...
            // compute flow index 2
            hash(flow2,     HashAlgorithm.xor16, HASH_BASE, {hdr.ipv4.srcAddr, 7w11, hdr.ipv4.dstAddr}, HASH_MAX);
            hash(flow_opp2, HashAlgorithm.xor16, HASH_BASE, {hdr.ipv4.dstAddr, 7w11, hdr.ipv4.srcAddr}, HASH_MAX);
            ...
            //read packet count index 2
            pkt_count_2.read(last_pkt_cnt2,     flow2);
            pkt_count_2.read(last_pkt_cnt_opp2, flow_opp2);
            ...
            //updating the packet count index 2
            last_pkt_cnt2 = last_pkt_cnt2 + 1;
            pkt_count_2.write(flow2, last_pkt_cnt2);
            ...
            find_min(meta.min_flow,last_pkt_cnt2,meta.min_flow_opp,last_pkt_cnt_opp2);
            ...
            if (diff_time > (bit<48>)WINDOW_SIZE) {
                ...
                pkt_count_2.write(flow2,(bit<48>)0);
                pkt_count_2.write(flow_opp2,(bit<48>)0);
