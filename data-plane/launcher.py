...
parser.add_argument('--cli', help='Path to BM CLI',
                    type=str, action="store", required=True)
#parser.add_argument('--debugger', help='Start with debugger', 
#                    type=bool, nargs='?', required=False, default=False)
...
class MyTopo(Topo):
    def __init__(self, sw_path, json_path, nb_hosts, nb_switches, links, **opts):
        ...
        for h in range(nb_hosts):
            self.addHost('h%d' % (h + 1), ip="192.168.1.%d/24" % (h + 1),
                    mac="00:00:00:00:01:0%d" % (h + 1))
