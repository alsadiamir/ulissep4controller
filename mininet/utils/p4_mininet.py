...
#
import os
os.sys.path.append('/home/prince7/.local/lib/python3.8/site-packages/')


from mininet.net import Mininet
from mininet.node import OVSSwitch, Switch, Host
...
class P4OVSSwitch(OVSSwitch):
    ...
    def __init__(self, name, sw_path = None, json_path = None,
        ...
        if device_id is not None:
            self.device_id = device_id
            P4OVSSwitch.device_id = max(P4OVSSwitch.device_id, device_id)
        else:
            self.device_id = P4OVSSwitch.device_id
            P4OVSSwitch.device_id += 1
        self.nanomsg = "ipc:///tmp/bm-{}-log.ipc".format(self.device_id)
...
class P4GrpcSwitch(Switch):
    ...
    def start( self, controllers ):
        ...
        if self.json_path:
            ...
            args.append("--no-p4")
        args.append("--log-flush --log-level trace --log-file log/%s.log" % self.name)
        if self.grpc_port:
