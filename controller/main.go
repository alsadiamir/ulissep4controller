...
func writeTunnelRules(p4RtC *client.Client, ing_port uint32, port uint32) {
	ing, _ := conversion.UInt32ToBinaryCompressed(ing_port)
	p, _ := conversion.UInt32ToBinaryCompressed(port)
	myTun := p4RtC.NewTableEntry(
		"MyIngress.myTunnel_exact",
		[]client.MatchInterface{&client.ExactMatch{
			Value: ing,
		}},
		p4RtC.NewTableActionDirect("MyIngress.myTunnel_forward", [][]byte{p}),
		nil,
	)

	if err := p4RtC.InsertTableEntry(myTun); err != nil {
		log.Errorf("Cannot insert entry in 'myTunnel_forward': %v", err)
	}
}
...
func initialize(p4RtC *client.Client, flag uint64) error {
	...
	*/
	if flag == 1 {
		...
		//writeTunnelRules(p4RtC,2,255)
		//sleep(5)
		pkt := owd.OWD{
			SrcMAC:       net.HardwareAddr(""),
			DstMAC:       net.HardwareAddr("ff:ff:ff:ff:ff:ff"),
			EthernetType: 4626,
			Pid:          0,
			Dst_id:       1,
			Nhop:         0,
		...
		port := []byte{0, 1}
		p4RtC.SendPacketOut(owd.Serialize(pkt), port)

	}
	if flag == 2 {
		writeTunnelRules(p4RtC, 1, 1)
		//writeTunnelRules(p4RtC,1,255)
	}
...
func handleStreamMessages(p4RtC *client.Client, messageCh <-chan *p4_v1.StreamMessageResponse) {
	for message := range messageCh {
		...
		case *p4_v1.StreamMessageResponse_Packet:
			...
			//if owdLayer := m.Layer(owd.OWD); owdLayer != nil {
			//fmt.Println("This is an OWD packet!")
			// Get actual TCP data from this layer
			//owd, _ := owdLayer.(*owd.OWD)
			pack := owd.Deserialize(m.Packet.GetPayload())
			fmt.Printf("pack = %s\n", pack.String())
			//fmt.Printf("m = %s\n", m.Packet.GetPayload())
			//}
